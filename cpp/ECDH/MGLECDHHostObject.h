...
#ifndef MGLECDHHostObject_h
#define MGLECDHHostObject_h
...
class MGLECDHHostObject : public MGLSmartHostObject {
// protected:
...
//  enum AuthTagState { kAuthTagUnknown, kAuthTagKnown, kAuthTagPassedToOpenSSL };
 ...
 public:
  ...
  // Without iv
  explicit MGLCipherHostObject(
      const std::string &curve_name, jsi::Runtime &runtime,
      std::shared_ptr<react::CallInvoker> jsCallInvoker,
      std::shared_ptr<DispatchQueue::dispatch_queue> workerQueue);

  void commonInit(jsi::Runtime &runtime, const char *curve_name);

  void installMethods();

  virtual ~MGLECDHHostObject();
